...
columns1 <- colnames(training)[-c(1:7)]
c <- apply(abc, 2, var, na.rm = TRUE )
...
modelRF <- randomForest(training$class ~ ., data=tmp, method="class")
predictionTest <- predict(modelRF,testing, type = 'class')
